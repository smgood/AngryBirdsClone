...
public class GameManager : MonoBehaviour
{
    ...
    void Update()
    {
        switch (CurrentGameState)
        {
            case GameState.Start:
                //if player clicks on angry bird, begin animating the bird 
                //to the slingshot
				ClickOnBird();
                break;
			case GameState.SelectBird:
				ClickOnBird();
				break;
            ...
            setOnCompleteHandler((x) =>
                        {
                            ...
                            {
							CurrentGameState = GameState.SelectBird;
    ...
    {
        CurrentGameState = GameState.BirdMovingToSlingshot;
		CurrentBird.transform.positionTo
			(Vector2.Distance(CurrentBird.transform.position / 10,
                ...
                setOnCompleteHandler((x) =>
                        {
                            ...
                            slingshot.BirdToThrow = CurrentBird;
                        });
    }
    ...
    {
        cameraFollow.BirdToFollow = CurrentBird.transform;
    ...
    void OnGUI()
    {
        ...
        {
            case GameState.Start:
                GUI.Label(new Rect(80, 450, 200, 100), "Click on a bird to start");
                break;
            case GameState.Won:
                GUI.Label(new Rect(80, 450, 200, 100), "You won! Tap the screen to restart");
                break;
            case GameState.Lost:
                GUI.Label(new Rect(80, 450, 200, 100), "You lost! Tap the screen to restart");
                break;
            default:
                break;
        }
	...
	void ClickOnBird ()
	{
		if (Input.GetMouseButtonUp(0))
		{
			...
			if (hit.collider != null && hit.collider.gameObject.tag == "Bird")
			{
				CurrentBird = hit.collider.gameObject;
				AnimateBirdToSlingshot();
			}
		}
	}
