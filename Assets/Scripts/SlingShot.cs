...
public class SlingShot : MonoBehaviour
{
    ...
    void Update()
    {
        switch (slingshotState)
        {
            ...
            case SlingshotState.UserPulling:
                ...
                {
					Dots = new List<GameObject>(GameObject.FindGameObjectsWithTag("Dot"));
    ...
    void DisplayTrajectoryLineRenderer2(float distance)
    {
        ...
        for (int i = 1; i < segmentCount; i++)
        {
			...
			float time2 = i * Time.fixedDeltaTime * 30 / segVelocity.magnitude;
            segments[i] = segments[0] + segVelocity * time2 + 0.5f * (Physics2D.gravity * 1.0f) * Mathf.Pow(time2, 2);
        }

        for (int i = 0; i < segmentCount; i++) {
			GameObject trajectoryDot = Instantiate(Dot);
			trajectoryDot.transform.position = new Vector2(segments[i].x, segments [i].y);
			}
    }
